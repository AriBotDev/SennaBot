...
class ErrorHandler:
    ...
    @classmethod
    def handle_command_error(cls, interaction, error):
        """
        ...
        if isinstance(error, cls.PERMISSION_ERRORS):
            return cls._handle_permission_error(interaction, error)
        elif isinstance(error, cls.USER_ERRORS):
            return cls._handle_user_error(interaction, error)
        else:
            return cls._handle_unknown_error(interaction, error)
    ...
    async def _handle_unknown_error(cls, interaction, error):
        ...
        if cls._report_to_owner and cls._owner_id:
            try:
                ...
                debug.log(f"Error reporting to owner: {e}")
    ...
    def handle_event_error(cls, event_name, error, extra_context=None):
        ...
        if cls._report_to_owner and cls._owner_id:
            try:
                # This won't work directly - needs to be called from a coroutine or command
                # Just log that it should be reported
                debug.log(f"Error in {event_name} should be reported to owner")
            except Exception as e:
                debug.log(f"Error preparing owner report: {e}")
